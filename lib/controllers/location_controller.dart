...
import 'package:ifood_delivery/data/repository/location_repo.dart';
...
class LocationController extends GetxController implements GetxService {
  ...
  late Position _position ;//= Position(longitude: 0, latitude: 0, timestamp: DateTime.now(), accuracy: 1, altitude: 1, heading: 1, speed: 1, speedAccuracy: 1);// = Position(longitude: 45.521563, latitude: -122.677433, timestamp: DateTime.now(), accuracy: 1, altitude: 1, heading: 1, speed: 1, speedAccuracy: 1);
  late Position _pickPosition ;//= Position(longitude: 0, latitude: 0, timestamp: DateTime.now(), accuracy: 1, altitude: 1, heading: 1, speed: 1, speedAccuracy: 1);// = Position(longitude: 45.521563, latitude: -122.677433, timestamp: DateTime.now(), accuracy: 1, altitude: 1, heading: 1, speed: 1, speedAccuracy: 1);
  ...
  Future<AddressModel> getCurrentLocation(bool fromAddress,
    ...
    try {
      Position newLocalData = await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.high);//best
  ...
  Future<ResponseModel> updateAddress(AddressModel addressModel, int addressId) async {
    _isLoading = true;
    update();
    http.Response response = await locationRepo.updateAddress(addressModel, addressId);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      Map json = jsonDecode(response.body);
      getAddressList();
      responseModel = ResponseModel(true, json["message"]);
    } else {
  ...
  Future<String> getAddressFromGeocode(LatLng latLng) async {
    ...
    print(json);
    if (json["status"] == 'OK') {
      _address = json["results"][0]['formatted_address'].toString();
    } else {
  ...
  Future<ResponseModel> getZone(String lat, String long, bool markerLoad) async {
    ...
    http.Response response = await locationRepo.getZone(lat, long);

    if(response.statusCode == 200) {
      Map json = jsonDecode(response.body);
      _inZone = true;
      print(" zone id "+json['zone_id'].toString());
      if(json['message'].toString()=='success' ) {
        _responseModel = ResponseModel(true, json['zone_id'].toString());
      } else {
        _inZone = false;
        _responseModel = ResponseModel(false, json['zone_id'].toString());
      }

    }else {
      _inZone = false;
      _responseModel = ResponseModel(false, response.statusCode.toString());
    }
    if(markerLoad) {
      _loading = false;
    }else {
  ...
  Future<ResponseModel> addAddress(AddressModel addressModel) async {
    ...
    print("Save the address" +response.statusCode.toString());
    update();
    return responseModel;
  }
