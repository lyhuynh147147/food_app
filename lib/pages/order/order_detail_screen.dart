...
class OrderDetailPage extends StatefulWidget {
  ...
  final int orderId;
  OrderDetailPage({required this.orderModel, required this.orderId});
          ...
          return orderController.orderDetails.isNotEmpty ? Column(children: [
            ...
            Expanded(child: Scrollbar(child: SingleChildScrollView(
              ...
              child: Center(child: SizedBox(width: Dimensions.WEB_MAX_WIDTH, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Row(
                          children: [
                            Container(height: 7, width: 7, decoration: BoxDecoration(
                              color: (_order.orderStatus == 'thất bại'/*'failed'*/ || _order.orderStatus == 'hoàn trả'/*'refunded'*/) ? Colors.red : Colors.green,
                              shape: BoxShape.circle,
                            )),
                            SizedBox(width: Dimensions.height10,),
                            Text(
                              _order.orderStatus == 'đã giao hàng'/*'delivered'*/ ? '${'delivered_at'.tr} ${DateConverter.dateTimeStringToDateTime(_order.delivered!)}'
                ...
                (_order.orderNote  != null && _order.orderNote!.isNotEmpty) ? Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('additional_info'.tr, style: robotoRegular),
            ...
            !orderController.showCancelled ? Center(
              child: SizedBox(
                width: Dimensions.WEB_MAX_WIDTH,
                child: Row(children: [
                  (_order.orderStatus == 'chưa giải quyết'/*'pending*/ || _order.orderStatus == 'accepted' || _order.orderStatus == 'đã xác nhận'//'confirmed'
                  ...
                  _order.orderStatus == 'chưa giải quyết'/*'pending'*/ ? Expanded(child: Padding(
                    padding: EdgeInsets.all(Dimensions.PADDING_SIZE_SMALL),
                    child: TextButton(
            ...
            (_order.orderStatus == 'thất bại'/*'failed'*/ && Get.find<SplashController>().configModel?.cashOnDelivery!=null) ? Center(
              child: Container(
                width: Dimensions.WEB_MAX_WIDTH,
                padding: EdgeInsets.all(Dimensions.PADDING_SIZE_SMALL),
                child: CustomButton(
